 ...
 *
 */
public class OpenStreetMapWaysBulkLoadDriver extends Configured implements Tool 
{
	
	private static final String DATA_SEPERATOR = ";";
	
	private static final String TABLE_NAME = "ways";
	
	private static final String COLUMN_FAMILY_1="wayData";
	...
	public static void main(String[] args) 
	{
		...
		try
		{
			int response = ToolRunner.run(HBaseConfiguration.create(),
				new OpenStreetMapWaysBulkLoadDriver(), args);
...
//		configuration.set("COLUMN_FAMILY_2",COLUMN_FAMILY_2);
		...
		job.setJarByClass(OpenStreetMapWaysBulkLoadDriver.class);
		
		job.setInputFormatClass(TextInputFormat.class);
		
		job.setMapOutputKeyClass(ImmutableBytesWritable.class);
		
		job.setMapperClass(OpenStreetWaysMapper.class);
