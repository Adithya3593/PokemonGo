 ...
 *
 */
public class PokemonGoBulkLoadDriver extends Configured implements Tool 
{
	
	private static final String DATA_SEPERATOR = "\\t";
	
	private static final String TABLE_NAME = "pokemongo";
	
	private static final String COLUMN_FAMILY_1="cf";
	...
	public static void main(String[] args) 
	{
		...
		try
		{
			int response = ToolRunner.run(HBaseConfiguration.create(),
				new PokemonGoBulkLoadDriver(), args);
...
//		configuration.set("COLUMN_FAMILY_2",COLUMN_FAMILY_2);
		...
		job.setJarByClass(PokemonGoBulkLoadDriver.class);
		
		job.setInputFormatClass(TextInputFormat.class);
		
		job.setMapOutputKeyClass(ImmutableBytesWritable.class);
		
		job.setMapperClass(PokemonGoMapper.class);
